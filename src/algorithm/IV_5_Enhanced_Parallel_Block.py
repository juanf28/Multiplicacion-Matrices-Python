...
class IV_5_Enhanced_Parallel_Block:
    @staticmethod
    def _multiply_block(args):
        matrizA, matrizB, size1, size2, i1_start, i1_end = args
        matrizRes = [[0 for _ in range(size2)] for _ in range(size1)]

        for i1 in range(i1_start, i1_end, size2):
    ...
    @staticmethod
    def alg_IV_5_Enhanced_Parallel_Block(matrizA, matrizB, size1, size2):
        # Determine number of processes to use
        num_processes = 4  # Adjust according to your system's capabilities
        pool = Pool(processes=num_processes)

        # Define arguments for each block computation
        block_args = [(matrizA, matrizB, size1, size2, 0, size1 // 2),
                      (matrizA, matrizB, size1, size2, size1 // 2, size1)]

        # Parallel computation of block multiplication
        result_matrices = pool.map(IV_5_Enhanced_Parallel_Block._multiply_block, block_args)
    ...
    def multiply(matrizA, matrizB):
        N = len(matrizA)
        P = len(matrizB)
        return IV_5_Enhanced_Parallel_Block.alg_IV_5_Enhanced_Parallel_Block(matrizA, matrizB, N, P)
